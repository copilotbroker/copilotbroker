import { Calendar, Check, RefreshCw, Unplug, Wifi, WifiOff } from "lucide-react";
...
export function GoogleConnectCard({ connection }: GoogleConnectCardProps) {
  if (!connection) {
    return (
      <Card className="border-dashed border-2 border-muted-foreground/20 bg-muted/30">
        <CardContent className="flex flex-col items-center justify-center py-10 text-center gap-4">
          ...
          <Button disabled className="gap-2">
            ...
            <span className="text-xs opacity-60">(em breve)</span>
          </Button>
          <p className="text-xs text-muted-foreground">
            Integração com Google Calendar estará disponível em breve.
          </p>
        </CardContent>
      </Card>
    );
    ...
    <Card className="border-green-500/30 bg-green-500/5">
      <CardContent className="flex items-center justify-between py-4">
        ...
        <div className="flex gap-2">
          ...
          <Button variant="ghost" size="sm" disabled className="gap-1 text-destructive">
