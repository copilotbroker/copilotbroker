...
export function FollowUpSheet({
  open, onOpenChange, leadId, leadName, leadPhone, projectName, brokerName, brokerId, leadStatus, onCreated,
}: FollowUpSheetProps) {
  ...
  const addStep = () => setSteps((prev) => [...prev, { messageContent: "", delayMinutes: 5, sendIfReplied: false }]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const stepsToInsert = steps.map((step, i) => ({
        campaign_id: campaign.id, step_order: i + 1, message_content: step.messageContent,
        delay_minutes: i === 0 ? 0 : step.delayMinutes, send_if_replied: i === 0 ? true : step.sendIfReplied || false,
      }));
      ...
      await supabase.from("lead_interactions").insert([
        { lead_id: leadId, interaction_type: "status_change" as any, old_status: previousStatus as any, new_status: "awaiting_docs" as any, channel: "whatsapp", broker_id: brokerId, created_by: currentUser?.id, notes: `Lead movido para Copiloto Ativo ao ativar follow-up` },
        { lead_id: leadId, interaction_type: "note_added" as any, channel: "whatsapp", notes: `Follow-up WhatsApp agendado (${steps.length} etapas):\n\n${steps.map((s, i) => `Etapa ${i + 1}: ${s.messageContent}`).join("\n")}`, created_by: currentUser?.id }
      ]);
      ...
      <SheetContent className="w-full sm:max-w-lg bg-[#0f0f11] border-[#2a2a2e] flex flex-col h-full p-0">
        <div className="px-6 pt-6">
          <SheetHeader>
            <SheetTitle className="text-white flex items-center gap-2">
              <MessageCircle className="w-5 h-5 text-emerald-400" />Agendar Follow-Up
            </SheetTitle>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-2">
          ...
          <div className="mt-6 rounded-lg border border-[#2a2a2e] bg-[#1a1a1d] p-4 space-y-3">
            <div className="flex items-center justify-between">
              <Label className="text-sm font-medium text-slate-300 flex items-center gap-2">
                <Clock className="w-4 h-4 text-emerald-400" />Início do envio
              </Label>
              ...
              <div className="flex gap-3">
                <Popover open={calendarOpen} onOpenChange={setCalendarOpen}>
                  <PopoverTrigger asChild>
                    <Button variant="outline" className={cn("flex-1 justify-start text-left font-normal bg-[#0f0f11] border-[#2a2a2e] h-9 text-sm", !startDate && "text-muted-foreground")}>
                  ...
                  <PopoverContent className="w-auto p-0 bg-[#1e1e22] border-[#2a2a2e]" align="start">
                    <Calendar mode="single" selected={startDate} onSelect={(d) => { setStartDate(d); setCalendarOpen(false); }} initialFocus disabled={(date) => date < new Date(new Date().setHours(0, 0, 0, 0))} className={cn("p-3 pointer-events-auto")} />
                  </PopoverContent>
                </Popover>
                <Input type="time" value={startTime} onChange={(e) => setStartTime(e.target.value)} className="w-28 bg-[#0f0f11] border-[#2a2a2e] text-white h-9 text-sm" />
              </div>
            )}
          </div>
          ...
          <div className="mt-3 space-y-3">
            ...
            <p className="text-xs text-slate-400 text-center pt-1">{steps.length} etapa{steps.length > 1 ? "s" : ""} · 1 lead</p>
          </div>
        </div>
