...
export function CadenciaSheet({
  open, onOpenChange, leadId, leadName, leadPhone, projectName, brokerName, brokerId, leadStatus,
  cadenceName = "Cadência 10D", initialSteps, onCreated,
}: CadenciaSheetProps) {
  ...
  const handleSubmit = async () => {
    ...
    setIsCreating(true);
    try {
      const { data: existing } = await (supabase.from("whatsapp_campaigns").select("id") as any).eq("lead_id", leadId).eq("status", "running").limit(1);
      if (existing && existing.length > 0) { toast.error("Já existe uma cadência ativa para este lead"); setIsCreating(false); return; }

      const { data: instanceData } = await supabase.from("broker_whatsapp_instances").select("working_hours_start, working_hours_end").eq("broker_id", brokerId).single();
      ...
      const stepsToInsert = steps.map((step, i) => ({ campaign_id: campaign.id, step_order: i + 1, message_content: step.messageContent, delay_minutes: i === 0 ? 0 : step.delayMinutes, send_if_replied: i === 0 ? true : step.sendIfReplied }));
      ...
      await supabase.from("lead_interactions").insert({ lead_id: leadId, interaction_type: "atendimento_iniciado" as any, old_status: leadStatus as any, new_status: "awaiting_docs" as any, broker_id: cadBroker?.id, notes: `Lead movido para Copiloto Ativo ao ativar ${cadenceName} por ${cadBroker?.name || "corretor"}`, created_by: cadUser?.id });

      const stepsPreview = steps.map((s, i) => `Etapa ${i + 1} (${formatDelayHuman(i === 0 ? 0 : s.delayMinutes)}): ${s.messageContent}`).join("\n");
      await supabase.from("lead_interactions").insert({ lead_id: leadId, interaction_type: "note_added" as any, channel: "whatsapp", notes: `⚡ ${cadenceName} ativada (${steps.length} etapas):\n\n${stepsPreview}`, created_by: cadUser?.id });
      ...
      <SheetContent className="w-full sm:max-w-lg bg-[#0f0f11] border-[#2a2a2e] flex flex-col h-full p-0">
        <div className="px-6 pt-6">
          <SheetHeader>
            <SheetTitle className="text-white flex items-center gap-2">
              <Zap className="w-5 h-5 text-emerald-400" />{cadenceName}
            </SheetTitle>
        ...
        <div className="flex-1 overflow-y-auto px-6 pb-2">
          <div className="mt-4 space-y-3">
            {steps.map((step, index) => (
              <div key={index}>
                ...
                <div className="rounded-lg border border-[#2a2a2e] bg-[#1a1a1d] p-4 space-y-3">
                  ...
                  {index > 0 && (
                    <>
                      <div className="space-y-1.5">
                        ...
                        <DelayIntervalPicker valueInMinutes={step.delayMinutes} onChange={(v) => updateStep(index, { delayMinutes: v })} />
                      </div>
                      <RadioGroup value={step.sendIfReplied ? "true" : "false"} onValueChange={(val) => updateStep(index, { sendIfReplied: val === "true" })} className="space-y-1.5 py-1.5">
                  ...
                  <div className="space-y-1.5">
                    <Textarea value={step.messageContent} onChange={(e) => updateStep(index, { messageContent: e.target.value })} placeholder="Digite sua mensagem... Use {nome}, {empreendimento}, {corretor_nome}" className="bg-[#0f0f11] border-[#2a2a2e] text-white min-h-[80px] text-sm" />
            ...
            <p className="text-xs text-slate-400 text-center pt-1">{steps.length} etapa{steps.length > 1 ? "s" : ""} · Envio automático imediato</p>
          </div>
        </div>

        <div className="border-t border-[#2a2a2e] bg-[#0f0f11] px-6 py-4 flex gap-3 mt-auto">
          <Button variant="outline" onClick={() => onOpenChange(false)} className="flex-1 border-[#2a2a2e] text-slate-300">Cancelar</Button>
