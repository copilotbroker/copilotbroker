          ...
          <div className={`bg-background border-2 border-primary/30 rounded-lg p-8 mb-12 transition-all duration-1000 delay-200 ${isVisible ? "opacity-100 scale-100" : "opacity-0 scale-95"}`}>
            ...
            <div className="grid sm:grid-cols-2 gap-4">
              {priorities.map((item, index) => (
                <div
                  key={item.text}
                  className={`flex items-center gap-3 p-4 bg-card rounded-lg border border-border/50 transition-all duration-500 ${isVisible ? "opacity-100 translate-x-0" : "opacity-0 -translate-x-5"}`}
...
export default STUrgencySection;
