...
export interface FollowUpStats {
  ...
  lateResponses: number; // responded after 3+ touches
}
...
export function useBrokerDashboard(filters: BrokerDashboardFilters) {
  ...
  const funnelQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (projectId) {
        pvQuery = pvQuery.eq("project_id", projectId);
      }
      ...
      // 3. Count responded: leads that had at least one interaction (last_interaction_at > created_at)
  ...
  const followUpQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      // Group by conversation to find max attempt
      ...
      // Get conversations that had replies (last_message_direction = inbound)
      ...
      const responseRate = totalSent > 0 ? (totalReplied / convMap.size) * 100 : 0;
      const avgTouchesToReply = totalReplied > 0
        ? followupsArr
            .filter((f) => convIds.includes(f.conversation_id))
            .reduce((sum, f) => sum + f.attempt_number, 0) / totalReplied
        : 0;
  ...
  if (funnel) {
    ...
    if (funnel.visitors > 0 && funnel.leads > 0) {
      ...
      if (lpRate < 2) {
        insights.push({
          type: "warning",
          title: "Conversão da LP baixa",
          description: `Apenas ${lpRate.toFixed(1)}% dos visitantes se cadastraram. Revise o copy e a oferta da landing page.`,
        });
      }
    }
    ...
    if (funnel.leads > 10 && funnel.responded > 0) {
      ...
      if (responseRate < 30) {
        insights.push({
          ...
          description: `Apenas ${responseRate.toFixed(0)}% dos leads responderam. O follow-up pode precisar de ajustes.`,
        });
      }
    }
    ...
    if (funnel.scheduled > 0 && funnel.visited > 0) {
      ...
      if (visitRate < 50) {
        insights.push({
          ...
          description: `Apenas ${visitRate.toFixed(0)}% dos agendados compareceram. Considere lembrar os leads.`,
        });
      } else if (visitRate >= 80) {
        insights.push({
          type: "success",
          title: "Ótimo comparecimento",
          description: `${visitRate.toFixed(0)}% dos agendados comparecem — excelente taxa!`,
        });
      }
    }
    ...
    if (funnel.proposals > 0) {
      ...
      if (closeRate < 20) {
        insights.push({
          type: "warning",
          title: "Baixa conversão de proposta",
          description: `Apenas ${closeRate.toFixed(0)}% das propostas viraram venda. Revise a negociação.`,
        });
      }
    }
    ...
    if (funnel.leads === 0) {
      insights.push({
        ...
        description: "Nenhum lead foi registrado neste período. Verifique suas campanhas.",
      });
    }
  }
  ...
  if (followUp) {
    if (followUp.lateResponses > 3) {
      insights.push({
        type: "info",
        title: "Respostas tardias",
        description: `${followUp.lateResponses} leads responderam somente após 3+ toques. O follow-up automático está funcionando.`,
      });
    }
  }
